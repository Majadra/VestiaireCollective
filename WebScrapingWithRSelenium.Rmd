...
temp_time <- Sys.time()
### 9348
for (Index_product in 10804:11881)
{
  ...
  Price_sold[[Index_product]] <- page %>% html_nodes(".productPrice__sold") %>% html_text()
 ...
 if (length(Price_sold[[Index_product]]) == 0) 
 {
  ...
  #Let's see maybe we can get some insight ---
  ###################
...
write.csv(temp_df,"Sold_items_raw_database.csv",row.names = FALSE)
